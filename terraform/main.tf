...
resource "aws_route_table" "mp_public_rt" {
  vpc_id = aws_vpc.mp_vpc.id
...
resource "aws_route" "default_route" {
  route_table_id         = aws_route_table.mp_public_rt.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.mp_internet_gateway.id
}

resource "aws_route_table_association" "mp_public_assoc" {
  subnet_id      = aws_subnet.mp_public_subnet.id
  route_table_id = aws_route_table.mp_public_rt.id
}

resource "aws_security_group" "mp_sg" {
  name        = "public_sg"
...
resource "aws_instance" "dev_node" {
  ...
  key_name               = aws_key_pair.mp_auth.id
  vpc_security_group_ids = [aws_security_group.mp_sg.id]
  subnet_id              = aws_subnet.mp_public_subnet.id
